...
#Adding two integer variables for the number of full-time and part-time teams:
y1 ={d: m.addVar(vtype=GRB.INTEGER) for d in D}
y2 ={d: m.addVar(vtype=GRB.INTEGER) for d in D}

#Objective
m.setObjective(quicksum(CT[d][s]*X[d,s]*WD[s] for d in D for s in S) +
               quicksum(4500*y1[d]+ 2750*y2[d] for d in D),GRB.MINIMIZE)
...
print("\nFuLL TIME   PART TIME")
for d in D:
    print(round(y1[d].x),"         ",round(y2[d].x))
