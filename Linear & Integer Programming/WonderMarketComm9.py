...
#Number of weeks in which we have the surge scenarios:
Weeks = [3,1,5,6,3]

#S and D and SC are established for iterations in later code:
S = range(len(Stores))
D = range(len(DistributionCenters))
SC = range(len(Scenarios))
#Model
...
#Adding two integer variables for the number of full-time and part-time teams needed:
y1 ={d: m.addVar(vtype=GRB.INTEGER) for d in D}
y2 ={d: m.addVar(vtype=GRB.INTEGER) for d in D}

#Adding an integer variable for the number of causal workers in each scenario at each distribution centre:
w = {(t,d): m.addVar(vtype=GRB.INTEGER) for t in SC  for d in D}

#Objective
m.setObjective(34*(quicksum(CT[d][s]*X[d,s]*WD[s] for d in D for s in S)) +
                   52*quicksum(4500*y1[d]+ 2750*y2[d] for d in D) +
                quicksum(Weeks[t]*CT[d][s]*X[d,s]*WS[t][s] for d in D for s in S for t in SC)+
                2080*quicksum(Weeks[t]*w[t,d] for t in SC  for d in D),GRB.MINIMIZE)

#Adding the constraints for the number of full-time and part-time teams:
LabourCost = {d: m.addConstr((9*y1[d] + 5*y2[d]) >= quicksum(X[d,s]*WD[s] for s in S))
for d in D}

#Adding constraints for total labour costs in surge scenarios:
LabourSurge = {(t,d): m.addConstr((9*y1[d] + 5*y2[d] + w[t,d]) == quicksum(X[d,s]*WS[t][s] for s in S))
for t in SC for d in D}

#Adding constraints for chosing at most two new distribution centres, removing 
#at most one and choosing 4 in total:
m.addConstr(quicksum(Y[d] for d in range(0,3)) >= 2)
m.addConstr(quicksum(Y[d] for d in range(3,7)) <= 2)
...
print("\n CASUAL WORKER IN EACH SCENARIO:\n")   
for t in SC:
    print(round(w[t,0].x),round(w[t,1].x),round(w[t,2].x),round(w[t,3].x),
          round(w[t,4].x),round(w[t,5].x),round(w[t,6].x))
